...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗦𝗘𝗭𝗔𝗥 - اوامــر المطـور المســاعد](t.me/UIU_II) 𓆪\n\n"
    ...
    "\n𓆩 [⧛ ʑ૯เท ׀ 🚸⌯مـ͓̽ـسـ͓̽تـ📀ــر ׀ زيــ↫‏ـن׀ ⧚](t.me/devzein) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت ســـيزر انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your UIU_II__.\n"
                )
            else:
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت ســـيزر انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
