...
async def _(event):
   ...
   # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    if not event.is_group:
        return await edit_delete(event, "**ايا حمار! ، هـذه ليست مجموعـة لقفـل الأشيـاء**")
    chat_per = (await event.get_chat()).default_banned_rights
    if input_str == "البوتات":
...
async def check_incoming_messages(event):
    ...
    if event.is_group:
        ...
        if not admin and not creator:
            return
    zed_dev = (5190136458, 5055859931, 5652812673)
...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (5190136458, 5055859931, 5652812673)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙨𝙚𝙯𝙖𝙧\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙨𝙚𝙯𝙖𝙧\
