    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://graph.org/file/715be1fa3d1177407f30a.jpg",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس القائد❤️‍🩹 الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝗟𝗘𝗮𝗗𝗲𝗥𓅛", "https://t.me/V_P_N_8"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت القائد❤️‍🩹 المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "جروب السجـل القائد", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
